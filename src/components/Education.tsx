            ...
            {education.map((edu, i) => (
              <Reveal key={i} delay={i * 0.15}>
                <div className="relative pl-14 sm:pl-20">
                  <div className="absolute left-0 sm:left-[17px] top-1 w-9 h-9 rounded-full bg-surface-2 border-2 border-coral-600/30 flex items-center justify-center z-10">
                    <GraduationCap className="w-3.5 h-3.5 text-coral-500" />
                  </div>
                  <div className="glass-card p-6 sm:p-8">
                    <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-3 mb-4">
                      <div><h3 className="text-lg font-display font-semibold text-text-primary">{edu.institution}</h3><p className="text-coral-500 text-sm font-medium mt-0.5">{edu.degree}</p></div>
                      <span className="inline-block px-3 py-1 text-[11px] font-medium rounded-full bg-coral-600/10 text-coral-500 border border-coral-600/20 shrink-0 self-start">{edu.period}</span>
                    </div>
                    ...
                    <ul className="space-y-2.5">
                      {edu.achievements.map((ach, j) => (
                        <li key={j} className="flex items-start gap-3 text-sm text-text-secondary leading-relaxed">
                          <span className="w-1.5 h-1.5 rounded-full bg-coral-500/60 mt-2 shrink-0" />{ach}
                        </li>
                      ))}
                    </ul>
                  </div>
