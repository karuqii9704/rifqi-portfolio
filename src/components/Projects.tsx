...
const categoryGradients: Record<string, string> = {
  "Full-Stack": "from-jade-500 to-khaki-400",
  Backend: "from-jade-600 to-jade-400",
  IoT: "from-khaki-500 to-jade-500",
};
          ...
          {projects.map((proj, i) => (
            <Reveal key={i} delay={i * 0.06}>
              <div className="group bento-card h-full flex flex-col">
                <div className={`absolute top-0 left-0 right-0 h-[2px] bg-gradient-to-r ${categoryGradients[proj.category] || "from-jade-500 to-khaki-400"} opacity-40 group-hover:opacity-100 transition-opacity duration-500`} />
                <div className="w-11 h-11 rounded-xl bg-surface-3 flex items-center justify-center mb-5 group-hover:scale-110 transition-transform">
                  <Code2 className="w-5 h-5 text-jade-400" />
                </div>
                ...
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-display font-semibold text-text-primary group-hover:text-jade-400 transition-colors">{proj.name}</h3>
                ...
                <div className="flex items-center gap-4 pt-3 border-t border-border-default">
                  <button className="flex items-center gap-1.5 text-xs text-text-tertiary hover:text-jade-400 transition-colors"><ExternalLink className="w-3.5 h-3.5" />Live Demo</button>
                  <button className="flex items-center gap-1.5 text-xs text-text-tertiary hover:text-khaki-300 transition-colors"><Globe className="w-3.5 h-3.5" />Source</button>
                </div>
              </div>
            </Reveal>
          ))}
